...
def check_house_harmony(first, second, planet, house):
    if check_planet_in_house(first, second, planet, house):
        return {
            "house_planet_1": SUN,
            "house_planet_2": FIRST_HOUSE,
        }
    else:
        return False
...
with open('samples.csv', mode='r') as csv_file:
    ...
    for row in csv_reader:
        ...
        # Read House Harmony
        hhs = []
        check_house_harmony(first, second, SUN, FIRST_HOUSE, hhs):
        if hh:
            hhs.append(hh)

        if check_planet_in_house(first, second, SUN, SEVENTH_HOUSE):
            hhs.append({
                "house_planet_1": SUN,
                "house_planet_2": SEVENTH_HOUSE,
            })
            check_planet_in_house(first, second, MOON, FIRST_HOUSE)
            check_planet_in_house(first, second, MOON, SEVENTH_HOUSE)
            check_planet_in_house(first, second, VENUS, FIRST_HOUSE)
            check_planet_in_house(first, second, VENUS, SEVENTH_HOUSE)
            check_planet_in_house(first, second, JUPITER, SEVENTH_HOUSE)
            check_planet_in_house(first, second, ASC, SEVENTH_HOUSE)
